...
╚══════════════════════════════════════════════╝
    ...
    def backup_wizard(self):
        ...
        # Create backup manager - need to convert config to URL if needed
        if isinstance(storage_url, dict):
            # Build URL from config for BackupManager
            config = storage_url
            if config['type'] == 'ssh':
                backup_storage_url = f"ssh://{config['user']}@{config['host']}:{config.get('port', 22)}{config.get('path', '/')}"
            elif config['type'] == 'ftp':
                backup_storage_url = f"ftp://{config['user']}:{config['password']}@{config['host']}:{config.get('port', 21)}{config.get('path', '/')}"
            else:
                backup_storage_url = config.get('path', '/')
        else:
            backup_storage_url = storage_url

        backup_mgr = BackupManager(source_uri, backup_storage_url)

        # Perform backup
        result = backup_mgr.backup_database(database, collections)
    ...
    def restore_wizard(self):
        ...
        console.print("[bold]Select backup location:[/bold]\n")
        ...
        console.print("  [cyan]4.[/cyan] 🔄 Rsync source")

        storage_choice = Prompt.ask("\nChoose storage type", choices=["1", "2", "3", "4"])

        storage_url = None
        if storage_choice == "1":
            # Local directory
            default_dir = os.path.join(os.path.expanduser("~"), "mongo_backups")
            storage_url = Prompt.ask("Backup directory", default=default_dir)
            ...
            user = Prompt.ask("FTP user")
            password = Prompt.ask("FTP password", password=True)
            port = Prompt.ask("FTP port", default="21")
            path = Prompt.ask("Remote path", default="/")
            storage_url = f"ftp://{user}:{password}@{host}:{port}{path}"

        elif storage_choice == "4":
            # Rsync
            host = Prompt.ask("Rsync host")
            user = Prompt.ask("Rsync user", default="root")
            path = Prompt.ask("Remote path", default="/backups/mongodb")
            storage_url = f"rsync://{user}@{host}{path}"
        ...
        if not backups:
            console.print("[yellow]No backups found at this location[/yellow]")
            Prompt.ask("Press Enter to continue")
            return
